...
In conclusion, we were able to successfully replicate much of the work done in the paper, and our results reflect the findings of the original. We encountered some challenges which limited our reproductions, most notably 1) being unable to run the provided pipeline in the vignette, instead doing the process manually, and; 2) being unable to replicate all figures from the paper on all data due to computational limitations.
